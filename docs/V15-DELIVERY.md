...
## RF corner as the east-river stack (2026-09-09, evening)

Josh rejected the wedge and asked for a comparison against the released renderings. Native crops of B4, A3 and B6 (`reference-audit/crops/rf-corner-*.jpg`, on the dashboard Sources tab) show the right-field seating as a straight stack along the EAST river arcade running north from the clock tower to the foul pole: lower tier, two-storey glazed suite band, upper tier cantilevered over the arcade roof, top row meeting a terrace on the tower's north face, tower about two arcade bays wide. Every earlier version, including the authored V13/V14 banks and the V12 "tower end" tier, had put the seating south of the bowl facing north-west. `r15_rf_corner.py` now builds that stack (lower 12 rows from z 14 at x 101.5, suites z 19.6–25.6, upper 16 rows to z 34.8 at x 117.1, terrace at 34.8, y −64 to −6), slims the tower to 12 m about its centre, and fills the triangle between the bowl end and the stack with a 12-row corner wrap plus a concourse at z 19.2. `r15_seating.BUILD_RF_BANKS = False` stops the old banks being built. Chairs 32,183, contacts 2,400, route 515/0, arrival 105/0. Open: the concourse behind the wrap is a blank slab, the V14 bowl end wall rises to 48 m above the 35 m stack, and the scoreboard truss at the pole is not modelled.
