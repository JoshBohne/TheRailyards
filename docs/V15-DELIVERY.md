...
## RF corner: two rejected rebuilds, reverted (2026-09-09)

Two attempts to rebuild the right-field corner from the released views were rejected by Josh and reverted: a single wedge bank (rows parallel to the river gallery, top at the tower terrace) and an east-river two-tier stack with a slimmed tower and corner wrap. The corner is back to the two parallel-row RF banks with the terrace connector described above. What survives from the attempts: native RF-corner crops of B4, A3 and B6 in `reference-audit/crops/` (registered on the dashboard Sources tab), the `rf-source`, `rf-b4` and `rf-wrap` cameras in `render_v15.py`, and the `BUILD_RF_BANKS` switch in `r15_seating.py`. The comparison itself stands: the releases put the right-field seating along the east river arcade facing west, with the bowl's upper deck ending level with it and a slender tower. Neither attempt matched it well enough to keep.
