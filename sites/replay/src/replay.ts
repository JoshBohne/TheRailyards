...
export interface ReplayData {version:number;duration:number;sampleRate:number;contact:number;splash:number;waterCrossing:number;ballSamples:Point[];landing:Point;seatCount:number;instances:InstanceGroup[];waterDistanceFt:number;splashDistanceFt:number}
