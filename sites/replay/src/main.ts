...
async function start():Promise<void>{
 try{
  ...
  if(![10,11,12,13,14,15].includes(data.version)||!Array.isArray(data.ballSamples)||data.ballSamples.length<2||!Number.isFinite(data.duration)||data.duration<=0)throw new Error('Replay data is incomplete');
