...
out.with_suffix('.json').write_text(json.dumps({'scene':bpy.data.filepath,'sourceSceneSha256':hashlib.sha256(Path(bpy.data.filepath).read_bytes()).hexdigest(),'imageSha256':hashlib.sha256(out.read_bytes()).hexdigest(),'waterDistanceFt':data['waterDistanceFt'],'splashDistanceFt':data['splashDistanceFt'],'landing':data['landing'],'note':'Horizontal distance along the authored flight direction to the versioned river edge; illustrative geometry and flight, not a ballistics forecast.'},indent=2)+'\n')
