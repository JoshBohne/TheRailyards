...
views.update({name:{'position':p,'target':t,'lens':l} for name,p,t,l in [('home-plate',(-12,-15,30),(20,20,12),22),('above-diamond',(-54,-68,102),(46,32,19),32),('rf-corner',(158,15,63),(30,45,20),25),('east-bank-waterfront',(205,65,10),(48,65,24),24),('east-bank-terrace',(250,65,28),(35,65,24),32),('east-bank-north',(199,180,18),(30,48,27),27)]})
for name,v in views.items():
 cam.location=v['position'];cam.data.lens=v['lens'];cam.rotation_euler=(Vector(v['target'])-cam.location).to_track_quat('-Z','Y').to_euler();s.render.filepath=str(root.parent/'work/night-game/stills'/f'{name}.png');bpy.ops.render.render(write_still=True)
