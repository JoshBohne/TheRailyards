...
preset=os.environ.get('RAILYARDS_PRESET','south');apply_lighting(scene,preset)
...
for name in os.environ.get('RAILYARDS_INTERIOR_VIEWS',','.join(shots)).split(','):
    ...
    scene.render.use_border=False;scene.render.filepath=str(OUT/(os.environ.get('RAILYARDS_INTERIOR_PREFIX','interior-')+name+'.png'));t=time.monotonic();bpy.ops.render.render(write_still=True)
...
(OUT/(os.environ.get('RAILYARDS_INTERIOR_PREFIX','interior-')+'renders.json')).write_text(json.dumps(receipts,indent=2)+'\n');print(json.dumps(receipts))
