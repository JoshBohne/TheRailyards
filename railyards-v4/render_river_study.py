...
landing=Vector((142,30,0));start=Vector((0,0,13.0))
...
def path(t):return Vector((landing.x*t,landing.y*t,13*(1-t)+232*t*(1-t)))
...
for i in range(1,199):
 ...
 if ok and not any(word in obj.name.lower() for word in ['atmosphere','player','fan','spectator']):hits.append({'object':obj.name,'at':list(loc)})
...
count=int(os.environ.get('RAILYARDS_FRAMES','168'));dest=OUT/'review/v9/river';dest.mkdir(parents=True,exist_ok=True)
...
(OUT/'river-distance-study.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
